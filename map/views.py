...
def home_map(request):
    ...
    initialMap = folium.Map(location=[latitud, longitud], zoom_start=9,control_scale=True,)
    ...
    for idx, location in enumerate(locations, start=1):
        ...
        <strong>Numero de Bandejas:</strong> {location.n_bandejas}
    """, width=500, height=250)
        popup = folium.Popup(iframe, max_width=500)
        url = "https://test.eco-networks.com/wp-content/uploads/2024/01/{}".format
        ...
        for ruta in Ruta.objects.all():
            ...
            popup_html = f"<b>Nombre de la ruta: {ruta.nombre}</b><br>"\
...
def draw_map(request):
    if request.method == 'POST':
        try:
            ...
            Ruta.objects.create(nombre=nombre_ruta, geojson_data=geojson_data, color=color_ruta)

            return JsonResponse({'status': 'OK'})
        except Exception as e:
            return JsonResponse({'status': 'Error', 'message': str(e)})

    return JsonResponse({'status': 'Error', 'message': 'Solo se admiten solicitudes POST'})
...
def gestionar_fusiones(request, cluster_id):
    ...
    BandejaFormSet = modelformset_factory(Bandeja, form=BandejaForm, extra=cluster.n_fusiones, formset=BaseModelFormSet)

    if request.method == 'POST':
        formset = BandejaFormSet(request.POST, prefix='fusiones', queryset=Bandeja.objects.filter(cluster=cluster))
        ...
        formset = BandejaFormSet(queryset=Bandeja.objects.filter(cluster=cluster), prefix='fusiones')
...
""" def editar_fusiones(request, cluster_id):
    ...
    BandejaFormSet = modelformset_factory(Bandeja, form=BandejaForm, extra=0, formset=BaseModelFormSet)
...
def editar_fusiones(request, cluster_id):
    ...
    if request.method == 'POST':
        ...
        if formset.is_valid():
            formset.save()
            return redirect('map:view_clusters')  # Redirigir a la página deseada después de guardar
    else:
        formset = BandejaFormSet(queryset=Bandeja.objects.filter(cluster=cluster), prefix='fusiones')
